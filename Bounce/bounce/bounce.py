...
class Bounce:
    ...
    def __init__(
        ...
        #TODO:
        # with open(f"{self.results_dir}/outlog.txt", 'w+') as file:
        #     file.write("#Start")
        logging.getLogger().addHandler(logging.FileHandler(f"{self.results_dir}/outlog.txt"))
        ...
        while self._n_evals <= self.maximum_number_evaluations:
            ...
            if self.trust_region.terminated:
                if self.random_embedding.target_dim < self.benchmark.representation_dim:
                    ...
                    self.sample_init()
            # TODO: indented this one further to save after every sample
