...
class CustomVerify(Benchmark):
    def __init__(self):
        ...
        for index, (name, obj) in enumerate(dict(deserialized_conf).items()):
            ...
            if type(obj) == ConfigSpace.hyperparameters.categorical.CategoricalHyperparameter:
                ...
                # if is_binary:
                #     ...
                # Choices can be 1 and -1
                if obj.size == 2:
                    binary_parameters.append(Parameter(name=f"{name}", type=ParameterType.BINARY, lower_bound=0, upper_bound=1))
                    self.choice_dict[name] = obj.choices
                else:
                    categorical_parameters.append(Parameter(name=f"{name}", type=ParameterType.CATEGORICAL, lower_bound=0, upper_bound=obj.size - 1))
                    self.choice_dict[name] = obj.choices


            elif type(obj) == ConfigSpace.hyperparameters.uniform_integer.UniformIntegerHyperparameter:
                # TODO: categorical or continuous but int restrained...
                # breakpoint()
                self.int_restricted.add(name)
                discrete_parameters.append(
                    Parameter(name=f"{name}", type=ParameterType.CONTINUOUS, lower_bound=obj.lower, upper_bound=obj.upper))


            elif type(obj) == ConfigSpace.hyperparameters.uniform_float.UniformFloatHyperparameter:
                # TODO: log?
                continuous_parameters.append(
                    Parameter(name=f"{name}", type=ParameterType.CONTINUOUS, lower_bound=obj.lower, upper_bound=obj.upper))
                # breakpoint()


            else:
                # TODO: check for ordinal variables in th ecategorical ones
                print(f"unknown: {obj}")

        # breakpoint()

        parameters = binary_parameters + categorical_parameters + continuous_parameters + discrete_parameters
        # breakpoint()
        super().__init__(parameters=parameters, noise_std=None)
    ...
    def verify_instance(self, instance_loc: str, config_array, output_queue):
        ...
        for idx, value in enumerate(config_array):
            name = self.parameters[idx].name
            variabls.append(f"-{name}")
            if self.parameters[idx].type in [ParameterType.BINARY, ParameterType.CATEGORICAL]:
                # breakpoint()
                try:
                    variabls.append(f"{self.choice_dict[name][int(value)]}")
                except Exception as e:
                    print(e)
                    breakpoint()
            elif self.parameters[idx].type is ParameterType.CONTINUOUS:
                if name in self.int_restricted:
    ...
    def _verify_instances_config(self, single_config):
        ...
        while instance_index < len(self.all_files):
            ...
            if len(process_data_list) < max_processes:
                # self.verify_instance(self.all_files[instance_index], single_config, output_queue)
                # breakpoint()
    ...
    def __call__(
        ...
        for config in X:
            print(config)
            print("---------------------")
            # TODO: check this... I thought they didnt do onehot but did it just based on hamming of one iterm...
            # This explains why bounce explodes as the effective dimensionarilyt is actually 181 instead of 111 on this problem now
            start = 0
            _x = []
            for parameter in self.parameters:
                end = start + parameter.dims_required
                one_hot = config[start:end]
                # transform onehot to categorical
                cat = np.argmax(one_hot)
                _x.append(cat)
                start = end
            _x = np.array(_x)
            # breakpoint()
            # results.append(self._verify_instances_config(config))
            results.append(self._verify_instances_config(_x))
            # results.append(1)
