...
class AbstractWrapper(object):
    ...
    def __init__(self):
        ...
        self.RESULT_MAPPING = {'SUCCESS': "SUCCESS"}
    ...
    def main(self, argv=None):
        ...
        try:
            ...
            run_group.add_argument("--runsolver-path", dest="runsolver",
                                   default=os.path.join(os.path.join(os.path.dirname(__file__), "runsolver"),
                                                        "runsolver"),
                                   help="path to runsolver binary (if None, the runsolver is deactivated)")
            run_group.add_argument("--temp-file-dir", dest="tmp_dir", default="./logs",
                                   help="directory for temporary files (relative to -exec-dir in SMAC scenario)")
            run_group.add_argument("--mem-limit", dest="mem_limit", default=self._mem_limit, type=int,
                                   help="memory limit in MB")
            run_group.add_argument("--internal", dest="internal", default=False, action="store_true",
            ...
            run_group.add_argument("--ext-callstring", dest="ext_callstring", default=None,
            ...
            run_group.add_argument("--ext-parsing", dest="ext_parsing", default=None,
            ...
            # if args.runsolvtarget_argser != "None" and not os.path.isfile(args.runsolver) and not args.internal:
            #     self._ta_status = "ABORT"
            ...
            if args.ext_callstring:
                target_cmd = self.get_command_line_args_ext(runargs=runargs, config=config_dict,
            ...
            if args.ext_parsing:
                resultMap = self.process_results_ext(self._solver_file, {"exit_code": self._ta_exit_code},
    ...
    def call_target(self, target_cmd):
        ...
        self._watcher_file = NamedTemporaryFile(suffix=".log", prefix="watcher-%d-" % (random_id), dir=self._tmp_dir,
                                                delete=False)
        self._solver_file = NamedTemporaryFile(suffix=".log", prefix="solver-%d-" % (random_id), dir=self._tmp_dir,
                                               delete=False)
    ...
    def print_result_string(self):
        sys.stdout.write(
            "Result for SMAC: %s, %s, %s, %s, %s" % (self._ta_status, str(self._ta_runtime), str(self._ta_runlength),
    ...
    def cleanup(self):
        ...
        if (self._ta_status is "ABORT" or self._ta_status is "CRASHED"):
            if (len(self._ta_misc) == 0):
                if (self._ta_exit_code is None):
                    self._ta_misc = 'Problem with run. Exit code was undefined.'
                else:
...
class PipelineWrapper(AbstractWrapper):
    ...
    def process_results(self, filepointer, out_args):
        ...
        if (re.search('Optimal solution found', data)):
            resultMap['status'] = 'SUCCESS'
        elif (re.search('Model is infeasible', data)):
            resultMap['status'] = 'UNSAT'
        elif (re.search('infeasible or unbounded', data)):
            resultMap['status'] = 'UNSAT'
        # else:
