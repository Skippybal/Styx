...
for t in range(args.n_trials):
    ...
    elif args.problem == "MIP":
        from MIPVerify_casmo_func import MIPVerifyOracle
        f = MIPVerifyOracle()
    else:
    ...
    # breakpoint()
    #TODO: add fX to these tables
    # best_x_table = wandb.Table(columns=["run_id"] + f.names, log_mode="INCREMENTAL")
    # all_config_table = wandb.Table(columns=["run_id"] + f.names, log_mode="INCREMENTAL")
    best_x_table = wandb.Table(columns=["run_id"] + f.names, log_mode="MUTABLE")
    all_config_table = wandb.Table(columns=["run_id"] + f.names, log_mode="MUTABLE")
